...
namespace WTools
{
    public partial class FOverAcount : Form
    {
        ...
        private void FOverAcount_Load(object sender, EventArgs e)
        {
            ...
            cmd1.CommandText = "SELECT ISNULL(sum([Quty]*[Price]*[InOut]),0) FROM [OtherCost] where[InOut] = 1 and DATEDIFF(day, [Cdate], GETDATE())= 0";
            label9.Text = Convert.ToInt32(cmd1.ExecuteScalar()).ToString();
            cmd1.CommandText = "SELECT ISNULL(sum([Quty]*[Price]*[InOut]*-1),0) FROM [OtherCost] where[InOut] = -1 and DATEDIFF(day, [Cdate], GETDATE())= 0";
